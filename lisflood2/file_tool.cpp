...
void read_file_netCDF_start_UKCP(const char* ncfilename, const char* varname,
	NetCDFVariable* ncvar)
{
	int recid, recdimid, xdimid, ydimid, xid, yid, longitudeid, latitudeid;
	int retval;
	...
	if ((retval = nc_inq_varid(ncvar->ncid, "time", &recid)))
		ERR(retval);
	if ((retval = nc_inq_varid(ncvar->ncid, "grid_longitude", &xid)))
		ERR(retval);
	if ((retval = nc_inq_varid(ncvar->ncid, "grid_latitude", &yid)))
		ERR(retval);
	if ((retval = nc_inq_varid(ncvar->ncid, "longitude", &longitudeid)))
		ERR(retval);
	if ((retval = nc_inq_varid(ncvar->ncid, "latitude", &latitudeid)))
		ERR(retval);

	if ((retval = nc_inq_dimid(ncvar->ncid, "time", &recdimid)))
		ERR(retval); 
	if ((retval = nc_inq_dimid(ncvar->ncid, "grid_longitude", &xdimid)))
		ERR(retval); 
	if ((retval = nc_inq_dimid(ncvar->ncid, "grid_latitude", &ydimid)))
		ERR(retval); 
	
	if ((retval = nc_inq_dimlen(ncvar->ncid, recdimid, &ncvar->tlen)))
		ERR(retval); 
	if ((retval = nc_inq_dimlen(ncvar->ncid, xdimid, &ncvar->xlen)))
		ERR(retval); 
	if ((retval = nc_inq_dimlen(ncvar->ncid, ydimid, &ncvar->ylen)))
		ERR(retval); 
	...
	size_t start2[] = { 0 , 0 };
	size_t xycount[] = { ncvar->ylen, ncvar->xlen };
	ncvar->latitude_UKCP = (NUMERIC_TYPE*)calloc(ncvar->ylen*ncvar->xlen, sizeof(NUMERIC_TYPE));
	if (retval = NC_GET_VARA_NUMERIC_TYPE(ncvar->ncid, latitudeid, start2, xycount,
		ncvar->latitude_UKCP))
		ERR(retval);
	
	ncvar->longitude_UKCP = (NUMERIC_TYPE*)calloc(ncvar->ylen*ncvar->xlen, sizeof(NUMERIC_TYPE));
	if (retval = NC_GET_VARA_NUMERIC_TYPE(ncvar->ncid, longitudeid, start2, xycount,
		ncvar->longitude_UKCP))
		ERR(retval);
}
...
bool read_file_netCDF(NetCDFVariable *ncvar, NUMERIC_TYPE curr_time)
{
  ...
  if (old_idx != ncvar->time_idx) {
	...
	if (ncvar->UKCP == ON)
	{
		size_t start[] = { 0, ncvar->time_idx, 0, 0 };
		size_t count[] = { 1, 1, ncvar->ylen, ncvar->xlen};
		float* pr_tmp;
		pr_tmp = (float*)calloc(ncvar->ylen * ncvar->xlen, sizeof(float));
		
		if (retval = nc_get_vara_float(ncvar->ncid, ncvar->varid, start,
			count, pr_tmp)) ERR(retval);
		for (int k = 0; k < ncvar->ylen * ncvar->xlen; k++)
		{
			ncvar->data[k] = (NUMERIC_TYPE)pr_tmp[k];
		}
