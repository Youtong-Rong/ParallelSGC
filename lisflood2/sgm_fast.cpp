...
inline void ProcessSubGridQBlock(const int block_index, const int grid_cols_padded,
	...
	NUMERIC_TYPE* SGC_Qx_grid, NUMERIC_TYPE* SGC_Qx_old_grid, NUMERIC_TYPE* SGC_Qy_grid, NUMERIC_TYPE* SGC_Qy_old_grid,
			...
			h1 + bankFullHeight1 > depth_thresh)
		{
			...
			if (hflow > depth_thresh)
			{
				...
				if (grid_index1 - grid_index0 == 1)
				{
					...
					theta_fix = getmax(theta_fix, 0.7);
...
#pragma simd // note this pragma is here to hint the compiler that this should be vectorized - the compiler will warn if not vectorized
	for (int i = row_start_x; i < row_end_x; i++)
	{
		...
		if (hflow > depth_thresh)// && (h0 > depth_thresh || h1 > depth_thresh))
		{
			...
			q_tmp = CalculateQ(surface_slope, hflow, delta_time, g, area, g_friction_sq_x_grid[index_next], qc, max_Froude);
		}
		else
		{
			surface_slope = C(0.0);
...
#pragma simd
	for (int i = row_start_y; i < row_end_y; i++)
	{
		...
		if (hflow > depth_thresh)// && (h0 > depth_thresh || h1 > depth_thresh))
		{
			...
			q_tmp = CalculateQ(surface_slope, hflow, delta_time, g, area, g_friction_sq_y_grid[index_next], qc, max_Froude);
		}
		else
		{
			surface_slope = C(0.0);
...
inline NUMERIC_TYPE SGC2_Infil_Rate_floodplain_row(const int j,
		const int dem_row_start, int dem_row_end,
		const int row_start, int row_end,
		const int grid_row_index, const int grid_cols_padded,
		const NUMERIC_TYPE depth_thresh,
		const NUMERIC_TYPE row_cell_area,
		const NUMERIC_TYPE evap_deltaH_step,
		const NUMERIC_TYPE* evap_row,
		NUMERIC_TYPE* h_row,
		const NUMERIC_TYPE* rain_row,
		const NUMERIC_TYPE* Qx_grid, const NUMERIC_TYPE* Qy_grid,
		const NUMERIC_TYPE* infil_row, NUMERIC_TYPE* volume_grid,
		NUMERIC_TYPE* volume_row, NUMERIC_TYPE delta_time,
		Arrays* Arrptr,
		Pars* Parptr,
		Solver* Solverptr)
	{
		...
		NUMERIC_TYPE reduce_evap_loss = C(0.0);
		//NUMERIC_TYPE inflow = C(0.0);
		NUMERIC_TYPE wet_time_step = Solverptr->wet_infil_time;
#pragma ivdep
#pragma simd
		for (int i = dem_row_start; i < dem_row_end; i++)
		{
			NUMERIC_TYPE h_new, dV = C(0.0), dInflow_Vol = C(0.0);
			NUMERIC_TYPE h_old = h_row[i];
			int index = grid_row_index + i;
			NUMERIC_TYPE evap_deltaV_step = evap_row[i] * row_cell_area;
			NUMERIC_TYPE infil_deltaV_step = C(0.0);
			if(infil_row[i]>C(0.0))	infil_deltaV_step = infil_row[i] * delta_time * row_cell_area;  // infil rate would be -9999 for subgrid cells and no soil-type definition area
			NUMERIC_TYPE inflow = Get_discharge_inflow(grid_row_index, grid_cols_padded, i, Qx_grid, Qy_grid) / row_cell_area;
			NUMERIC_TYPE inflow_vol = Get_discharge_inflow(grid_row_index, grid_cols_padded, i, Qx_grid, Qy_grid)*delta_time;
			NUMERIC_TYPE surfMoisture = inflow + rain_row[i] / row_cell_area + h_row[i] / delta_time;
			NUMERIC_TYPE surfVolume = inflow_vol + rain_row[i] * delta_time + h_row[i] * row_cell_area;
#ifdef _DEBUG
			if (Arrptr->SGCwidth[Parptr->xsz * j + i] == C(0.0) && surfVolume < 0)
				printf("surfVolume <0 %" NUM_FMT" (%d,%d) \n", surfVolume, i, j);             //for standard grid, surfVolume should be positive.
#endif													      //negative volume indicates that the discharge output larget than the available volume in a grid.
			
			
			if (Arrptr->SGCwidth[Parptr->xsz * j + i] == C(0.0))
			{
				//if (infil_row[i]+evap_row[i] >= surfMoisture)                                 // explain why the evap_row here. Only GrnAmpt infil, evap_row = 0; Only evap, no this calculation. 
				if(infil_deltaV_step + evap_deltaV_step >= surfVolume)
				{                                                                //if both evap and GrnAmpt are used, the delta_vol can be updated in SGC2_Evaporation_floodplain_row(),
					//dV = delta_volume_row[i] + volume_grid[grid_row_index + i]+inflow * row_cell_area * delta_time;  //delta_vol store the rainfall at this time step. In this step, if infil+Evap > surfMoisture, then all the water become mass loss;
					reduce_evap_loss += (inflow_vol + volume_row[i]+ volume_grid[grid_row_index + i]);
					volume_row[i] = C(0.0);                      //rainfall volume                                      //if infil+evap <surfMoisture, the delta_vol continues to lose the infil volume.
					volume_grid[grid_row_index + i] = C(0.0);   //grid volume
					dV = inflow_vol;
				}
				else
				{
					dV = infil_deltaV_step + evap_deltaV_step;
					reduce_evap_loss += dV;
				}
			}
	...
	void Initialize_GrnAmpt_Soiltype(TGrnAmpt* Infilptr, Pars* Parptr, const NUMERIC_TYPE* soiltype_grid, const int grid_row_index, const int grid_cols_padded)
	{
		
		for (int i = 0; i < grid_cols_padded; i++)
		{
			int index = grid_row_index + i;
			int soiltype = int(soiltype_grid[index]);
			Get_Soil_Type(soiltype, Infilptr, index);
			Infilptr->Lu[index] = 4 * sqrt(25.4 * 3.6) * sqrt(Infilptr->Ks[index]);      // Oriigina equation Lu=4*sqrt(Ks), Lu is in the unit of inch,
			Infilptr->Fumax[index] = Infilptr->IMDmax[index] * Infilptr->Lu[index];      // and Ks is in in/hr, and the constant is in (in-hr)^1/2.
			Infilptr->Infil_rate[index] = C(0.0);                                        // 4 (in-hr)^(1/2)=4* (25.4/1000 m * 3600s)^(1/2)
			GrnAmpt_initState(Infilptr, index);
		}
	}

	inline void SGC2_GrnAmpt_Infil_floodplain_row(
		const int row_start, int row_end,
		const NUMERIC_TYPE depth_thresh,
		const NUMERIC_TYPE row_cell_area,
		const NUMERIC_TYPE evap_deltaH_step,
		...
		NUMERIC_TYPE* volume_row,
		const NUMERIC_TYPE* Qx_grid, const NUMERIC_TYPE* Qy_grid, const NUMERIC_TYPE* soiltype_grid,
		const int grid_row_index, const int grid_cols_padded, const int j,
		Solver* Solverptr,
		Pars* Parptr,
		Arrays* Arrptr,
		TGrnAmpt *Infilptr, bool last_row)
	{
#ifdef __INTEL_COMPILER
		__assume_aligned(h_row, 64);
		__assume_aligned(volume_row, 64);
		...
		NUMERIC_TYPE reduce_evap_loss = C(0.0);
		NUMERIC_TYPE min_infil_time_step = Solverptr->wet_infil_time;
		NUMERIC_TYPE wet_infil_step = Solverptr->wet_infil_time;
		NUMERIC_TYPE dry_infil_step = Solverptr->dry_infil_time;
...
#pragma simd
		...
		{
			NUMERIC_TYPE h_new, dV = C(0.0);
			NUMERIC_TYPE rain_rate = C(0.0);
			int index = grid_row_index + i;
			NUMERIC_TYPE h_old = h_row[i];
			NUMERIC_TYPE Infil_rate = C(0.0);
			NUMERIC_TYPE curr_infil_time_step=C(0.0);
			last_col = (i == row_end - 1) && (last_row);
			NUMERIC_TYPE inflow = C(0.0), Surface_moisture = C(0.0);
			NUMERIC_TYPE new_infil_time = Solverptr->new_infil_time[j];

			//if (curr_time + delta_time > new_infil_time)
			//{
			


			inflow = Get_discharge_inflow(grid_row_index,grid_cols_padded, i, Qx_grid,Qy_grid)/row_cell_area;
			/*if (new_infil_time <= C(0.0))
			{
				enum SoilTypeId soiltype = Sand;
				Get_Soil_Type(soiltype, Infilptr, index);
				Infilptr->Lu[index] = 4 * sqrt(25.4 * 3.6) * sqrt(Infilptr->Ks[index]);
				Infilptr->Fumax[index] = Infilptr->IMDmax[index] * Infilptr->Lu[index];
				Infilptr->Infil_rate[index] = C(0.0);
				GrnAmpt_initState(Infilptr, index);
			}*/
			/*if (rain_row[i] > 0 || h_row[i] > 0)
			{
				new_infil_time += wet_infil_step;
				curr_infil_time_step = wet_infil_step;
			}
			else
			{
				new_infil_time += dry_infil_step;
				curr_infil_time_step = dry_infil_step;
			}*/
			new_infil_time += wet_infil_step;
			curr_infil_time_step = wet_infil_step;
			if (new_infil_time > Solverptr->Sim_Time)
			{
				curr_infil_time_step = Solverptr->Sim_Time - Solverptr->new_infil_time[j];
				new_infil_time = Solverptr->Sim_Time;
			}
			//if (curr_infil_time_step < min_infil_time_step) { min_infil_time_step = curr_infil_time_step; }
			//Solverptr->new_infil_time = new_infil_time;
			rain_rate = rain_row[i] / row_cell_area;
			if (int(soiltype_grid[index]) < 1 || int(soiltype_grid[index]) > 12|| Arrptr->SGCwidth[Parptr->xsz*j+i]>C(0.0))
			{
				Infil_rate =  NULLVAL;
			}
			else
			{
				Infil_rate = GrnAmpt_infil_rate(Infilptr, rain_rate, inflow, h_row, i, index, curr_infil_time_step, depth_thresh);
			}
			//Surface_moisture = h_row[i] / curr_infil_time_step + inflow + rain_row[i];
			Infilptr->Infil_rate[index] = Infil_rate;
			//}
			//if (last_col) 
			//{ 
			//	//cout <<"curr_time is "<<curr_time << "  infiltration rate is " << Infil_rate << endl; 
			//	ofstream ofs;
			//	string filename = "wet_time_step" + to_string(int(wet_infil_step)) + ".txt";
			//	ofs.open(filename, ios::app);
			//	ofs << curr_time << "   " << Infil_rate  << endl;
			//	ofs.close();
			//}
			//NUMERIC_TYPE evap_deltaV_step = Infil_rate *delta_time* row_cell_area;
			//if (h_old > depth_thresh) // There is water to evaporate on the flood plain
			//{
			//	// update depth by subtracting evap depth
			//	h_new = h_old - Infil_rate * delta_time;
			//	//check for -ve depths
			//	if (h_new < C(0.0))
			//	{
			//		// reduce evap loss to account for dry bed (don't go below 0)
		...
		//if (min_infil_time_step <= C(0.0)) { min_infil_time_step = dry_infil_step; }

		//if (last_col) 
		//{ 
			Solverptr->new_infil_time[j] += min_infil_time_step; 
		//}
		
	}
	
	void Get_Soil_Type(int soiltype, TGrnAmpt * Infilptr, const int index)
	{
		switch (soiltype)
		{
		case NONE: 
		{
			Infilptr->IMDmax[index] = C(0.385);                  //if soil type is not in the 11 kinds of soil, even we initialize these three parameters, we 
			Infilptr->S[index] = C(316.3) / 1000;                //do not calculate the infiltration rate. We set the infiltration rate to 0 directly without GrnAmpt.
			Infilptr->Ks[index] = C(0.3) / (1000 * 3600);
			
		}
		break;
		case Sand:
		{
			Infilptr->IMDmax[index] = C(0.417);
			Infilptr->S[index] = C(49.5) / 1000;
			Infilptr->Ks[index] = C(117.8) / (1000 * 3600);

		}
		break;
		case Loamy_sand:
		{
			Infilptr->IMDmax[index] = C(0.401);
			Infilptr->S[index] = C(61.3) / 1000;
			Infilptr->Ks[index] = C(29.9) / (1000 * 3600);
		}break;
		case Sandy_loam:
		{
			Infilptr->IMDmax[index] = C(0.412);
			Infilptr->S[index] = C(110.1) / 1000;
			Infilptr->Ks[index] = C(10.9) / (1000 * 3600);
		}break;
		case Loam:
		{
			Infilptr->IMDmax[index] = C(0.434);
			Infilptr->S[index] = C(88.9) / 1000;
			Infilptr->Ks[index] = C(3.4) / (1000 * 3600);
		}break;
		case Silt_loam:                                          //both the Silt and the Silt_loam share the same parameters. 
		case Silt:                                               //From the soil triangle, the silt is at the corner and has only one neighbour - Silt_loam
		{
			Infilptr->IMDmax[index] = C(0.486);
			Infilptr->S[index] = C(166.8) / 1000;
			Infilptr->Ks[index] = C(6.5) / (1000 * 3600);
		}break;
		case Sandy_clay_loam:
		{
			Infilptr->IMDmax[index] = C(0.330);
			Infilptr->S[index] = C(218.5) / 1000;
			Infilptr->Ks[index] = C(1.5) / (1000 * 3600);
		}break;
		case Clay_loam:
		{
			Infilptr->IMDmax[index] = C(0.309);
			Infilptr->S[index] = C(208.8) / 1000;
			Infilptr->Ks[index] = C(1.0) / (1000 * 3600);
		}break;
		case Silty_clay_loam:
		{
			Infilptr->IMDmax[index] = C(0.432);
			Infilptr->S[index] = C(273.0) / 1000;
			Infilptr->Ks[index] = C(1.0) / (1000 * 3600);
		}break;
		case Sandy_clay:
		{
			Infilptr->IMDmax[index] = C(0.321);
			Infilptr->S[index] = C(239.0) / 1000;
			Infilptr->Ks[index] = C(0.6) / (1000 * 3600);
		}break;
		case Silty_clay:
		{
			Infilptr->IMDmax[index] = C(0.423);
			Infilptr->S[index] = C(292.2) / 1000;
			Infilptr->Ks[index] = C(0.5) / (1000 * 3600);
		}break;
		case Clay:
		{
			Infilptr->IMDmax[index] = C(0.385);
			Infilptr->S[index] = C(316.3) / 1000;
			Infilptr->Ks[index] = C(0.3) / (1000 * 3600);
		}break;
	...
	NUMERIC_TYPE Get_discharge_inflow(const int grid_row_index, const int grid_cols_padded, int i,const NUMERIC_TYPE* Qx_grid, const NUMERIC_TYPE* Qy_grid)
	{
		int index = grid_row_index + i;
		int index_right = index + 1;
		int index_below = index + (grid_cols_padded);
		NUMERIC_TYPE inflow = (Qx_grid[index] - Qx_grid[index_right] + Qy_grid[index] - Qy_grid[index_below]); 
		//if (inflow <= C(0.0)) { inflow = C(0.0); }
		return inflow;	
	}

	void GrnAmpt_initState(TGrnAmpt* Infilptr, const int index) 
	{
		Infilptr->IMD[index] = Infilptr->IMDmax[index];
		Infilptr->Fu[index] = C(0.0);
		Infilptr->F[index] = C(0.0);
		Infilptr->Sat[index] = C(0.0);
		Infilptr->T[index] = C(0.0);
	}
	NUMERIC_TYPE GrnAmpt_infil_rate(TGrnAmpt* Infilptr, NUMERIC_TYPE rain_rate, const NUMERIC_TYPE inflow, const NUMERIC_TYPE* h_row, int i,const int index, NUMERIC_TYPE curr_infil_time_step, const NUMERIC_TYPE depth_thresh)
	{
		NUMERIC_TYPE irate = rain_rate + inflow;   //irate = net "rainfall" rate to upper zone (m/s);  irate= rainfall + runon
		NUMERIC_TYPE depth = h_row[i];
		NUMERIC_TYPE tstep = curr_infil_time_step;
		Infilptr->Fumax[index] = Infilptr->IMDmax[index] * Infilptr->Lu[index];
		NUMERIC_TYPE Fumax = Infilptr->Fumax[index];
		Infilptr->T[index] -= tstep;
		if (Infilptr->Sat[index]>C(0.0)) return GrnAmpt_Sat_Infil_rate(Infilptr, tstep, irate, depth,index);
		else return GrnAmpt_Unsat_infil_rate(Infilptr,tstep,irate, depth, index,depth_thresh);

	}

	NUMERIC_TYPE GrnAmpt_Unsat_infil_rate(TGrnAmpt* Infilptr, NUMERIC_TYPE tstep, NUMERIC_TYPE irate, NUMERIC_TYPE depth,const int index, const NUMERIC_TYPE depth_thresh)
		//
		//  Input:   infil = ptr. to Green-Ampt infiltration object
		//           tstep =  runoff time step (s),
		//           irate = net "rainfall" rate to upper zone (m/s);
		//                 = rainfall + snowmelt + runon,
		//                   does not include ponded water (added on below)
		//           depth = depth of ponded water (m)
		//           modelType = either GREEN_AMPT or MOD_GREEN_AMPT
		//  Output:  returns infiltration rate (m/s)
		//  Purpose: computes Green-Ampt infiltration when upper soil zone is
		//           unsaturated.
		//
	{
		double ia, c1, F2, dF, Fs, kr, ts;
		double ks = Infilptr->Ks[index];                                       
		double lu = Infilptr->Lu[index];                                

		// --- get available infiltration rate (rainfall + ponded water)
		ia = irate + depth / tstep;
		if (ia < C(0.0)) ia = C(0.0);

		// --- no rainfall so recover upper zone moisture
		if (ia == C(0.0))          
		{
			if (Infilptr->Fu[index] <= C(0.0)) return C(0.0);
			kr = lu / (300.*25.4*3.6);                           //The original equation is kr = Lu/300, Lu is in inch and Kr is in 1/hr. 
			dF = kr * Infilptr->Fumax[index] * tstep;            // The constant 300 has the unit of inch-hour. 300 in-hr = 300*25.4/1000 m *3600s
			Infilptr->F[index] -= dF;                            //therefore, now in the equation Lu is in m and kr is in 1/s. 
			Infilptr->Fu[index] -= dF;
			if (Infilptr->Fu[index] <= 0.0)
			{
				Infilptr->Fu[index] = 0.0;
				Infilptr->F[index] = 0.0;
				Infilptr->IMD[index] = Infilptr->IMDmax[index];
				return 0.0;
			}

			// --- if new wet event begins then reset IMD & F
			if (Infilptr->T[index] <= C(0.0))
			{
				Infilptr->IMD[index] = (Infilptr->Fumax[index] - Infilptr->Fu[index]) / lu;
				Infilptr->F[index] = 0.0;
			}
			return 0.0;
		}
		...
		if (ia <= ks)
		{
			...
			Infilptr->F[index] += dF;
			Infilptr->Fu[index] += dF;
			Infilptr->Fu[index] = min(Infilptr->Fu[index], Infilptr->Fumax[index]);
			if (Infilptr->T[index] <= 0.0)
			{
				Infilptr->IMD[index] = (Infilptr->Fumax[index] - Infilptr->Fu[index]) / lu;
				Infilptr->F[index] = 0.0;
			}
		...
		Infilptr->T[index] = (18.*25.4*3.6) / lu;                                //The original equation is T = 18/Lu, and the constant 18 has the unit of in-hr.
																				 //18 in-hr = 18*1/12 ft*3600s = 5400 ft-s = 18*25.4/1000 m *3600 s
		// --- find volume needed to saturate surface layer
		Fs = ks * (Infilptr->S[index] + depth) * Infilptr->IMD[index] / (ia - ks);

		// --- surface layer already saturated
		if (Infilptr->F[index] > Fs)
		{
			Infilptr->Sat[index] = C(1.0);
			return GrnAmpt_Sat_Infil_rate(Infilptr, tstep, irate, depth,index);
		}

		// --- surface layer remains unsaturated
		if (Infilptr->F[index] + ia * tstep < Fs)
		{
			dF = ia * tstep;
			Infilptr->F[index] += dF;
			Infilptr->Fu[index] += dF;
	...
	NUMERIC_TYPE GrnAmpt_Sat_Infil_rate(TGrnAmpt* Infilptr, NUMERIC_TYPE tstep, NUMERIC_TYPE irate, NUMERIC_TYPE depth, const int index)
	{
		double ia, c1, dF, F2;
		double ks = Infilptr->Ks[index];
		double lu = Infilptr->Lu[index];
		...
		ia = irate + depth / tstep;
		if (ia < C(0.0)) return C(0.0);

		// --- re-set new event recovery time
		Infilptr->T[index] = C(18.)*C(25.4)*C(3.6) / lu;      //The original equation is T = 18/Lu. T has the unit of hour, and Lu is in inch.
												    //The constant 18 has the unit of in-hr. 18 in-hr = 18*25.4/1000*3600 m/s;

		// --- solve G-A equation for new cumulative infiltration volume (F2)
		c1 = (Infilptr->S[index] + depth) * Infilptr->IMD[index];
		F2 = GrnAmpt_GetF2(Infilptr->F[index], c1, ks, tstep);
		dF = F2 - Infilptr->F[index];
	...
	NUMERIC_TYPE GrnAmpt_GetF2(NUMERIC_TYPE f1, NUMERIC_TYPE c1, NUMERIC_TYPE ks, NUMERIC_TYPE ts)
	{
		int    i;
		double f2 = f1;
		double f2min;
...
NUMERIC_TYPE SGC2_UpdateVolumeHeight_block(const int block_index,
	...
	const NUMERIC_TYPE delta_time, const NUMERIC_TYPE tstart, const NUMERIC_TYPE depth_thresh, const NUMERIC_TYPE g,
	...
	const NUMERIC_TYPE * dem_grid, const NUMERIC_TYPE* soiltype_grid,
	...
	const SGCprams *SGCptr,
	Arrays* Arrptr,
	TGrnAmpt *Infilptr,bool last_block,
...
//		__assume_aligned(sg_flow_ChannelRatio, 64);
		...
		bool GrnAmpt_Infil_evap_FLAG = false;
		bool Distributed_Infil_Evap_FLAG = false;
		if (Statesptr->calc_GrnAmpt_infiltration == ON)
		{
			GrnAmpt_Infil_evap_FLAG = true;
		}
		else
		{
			Distributed_Infil_Evap_FLAG = true;
		}
		
		// const_cell_evap always zero if evap Evaporation
		if (evap_deltaH_step > C(0.0) && Distributed_Infil_Evap_FLAG)
		{
		...
		if (Statesptr->calc_distributed_infiltration == ON && Distributed_Infil_Evap_FLAG)
		{
			int evap_row_start = wet_dry_bounds->fp_h[j].start;
			int evap_row_end = min(wet_dry_bounds->fp_h[j].end, wet_dry_bounds->dem_data[j].end);
			block_evap_loss += SGC2_Infil_floodplain_row(evap_row_start, evap_row_end, depth_thresh, row_cell_area, evap_deltaH_step,
				...
				delta_volume_row,delta_time);
		}
		
		
		
		if (Statesptr->calc_GrnAmpt_infiltration == ON && GrnAmpt_Infil_evap_FLAG)
		{
			if (curr_time<= tstart) { Initialize_GrnAmpt_Soiltype(Infilptr, Parptr, soiltype_grid,grid_row_index,grid_cols_padded); }
			
			bool last_row = (j == end_y - 1)&&(last_block);
			int evap_row_start = wet_dry_bounds->fp_h[j].start;
			int evap_row_end = min(wet_dry_bounds->fp_h[j].end, wet_dry_bounds->dem_data[j].end);

			int dem_row_start = wet_dry_bounds->dem_data[j].start;
			int dem_row_end = wet_dry_bounds->dem_data[j].end;
			
			
			while (Solverptr->new_infil_time[j] <= curr_time) {
				/*if (j == 0) {
					ofstream ofs;
					string filename = "waterd.txt"; 
					ofs.open(filename, ios::app);
					ofs << curr_time << "   " << h_grid[1] << endl;
					ofs.close();
				}*/
				
				SGC2_GrnAmpt_Infil_floodplain_row(dem_row_start, dem_row_end, depth_thresh, row_cell_area, evap_deltaH_step,
					h_grid + grid_row_index, // pointer to start of row
					rain_grid + grid_row_index,  // pointer to start of distributed infiltratioon grid
					delta_time, curr_time,
					delta_volume_row, Qx_grid, Qy_grid, soiltype_grid, grid_row_index, grid_cols_padded,j,
					Solverptr, Parptr, Arrptr,Infilptr, last_row);
			}

			block_evap_loss += SGC2_Infil_Rate_floodplain_row(j, dem_row_start, dem_row_end, evap_row_start, evap_row_end, grid_row_index, grid_cols_padded, depth_thresh, row_cell_area, evap_deltaH_step,
				evap_grid->data + grid_row_index, h_grid + grid_row_index, rain_grid + grid_row_index, Qx_grid, Qy_grid, Infilptr->Infil_rate + grid_row_index, volume_grid, delta_volume_row, delta_time,Arrptr, Parptr, Solverptr);
		}
	...
	for (int block_index = 0; block_index < wet_dry_bounds->block_count; block_index++)
	{
		...
		for (int j = start_y; j < end_y; j++)
		{
			...
			ProcessSubGridQBlock(j, grid_cols_padded, depth_thresh, delta_time, g, sub_grid_layout_rows, sub_grid_state_rows, SGCptr, h_grid,
				wet_dry_bounds, Qx_grid, Qy_grid, Qx_old_grid, Qy_old_grid, SGC_Qx_grid,SGC_Qx_old_grid, SGC_Qy_grid,SGC_Qy_old_grid,Parptr->dx,Parptr->dy, Parptr->max_Froude);
...
#pragma omp parallel for default(shared) reduction ( + : reduce_flood_area, reduce_domain_volume) schedule(static)
	...
	DynamicRain<> dynamic_rain(Fnameptr->dynamicrainfilename, Solverptr->t, Parptr->UKCP, verbose);
	if (Parptr->UKCP == ON) dynamic_rain.UKCP_generate_index_grid(grid_cols, grid_rows, grid_cols_padded, Parptr->blx, Parptr->tly, Parptr->dx, rain_grid_ind);
	...
	NUMERIC_TYPE tstart = Solverptr->t;
...
#pragma omp single
			{
			  ...
			  dynamic_rain.update_rain_grid_SGM(curr_time,tstart, rain_grid, dem_grid, wet_dry_bounds, dx_col, dy_col, Parptr->tly, grid_rows, grid_cols_padded, cell_area_col, Parptr->dx, Parptr->dy, Parptr->UKCP, rain_grid_ind);
			}
#endif
			...
			Do_Update(grid_cols, grid_rows, grid_cols_padded, delta_time,
				curr_time,tstart, depth_thresh, g, h_grid, volume_grid, Qx_grid, Qy_grid, Qx_old_grid, Qy_old_grid,SGC_Qx_grid,SGC_Qx_old_grid,SGC_Qy_grid,SGC_Qy_old_grid,
				...
				SGC_BankFullHeight_grid,
				dem_grid,soiltype_grid, g_friction_sq_x_grid, g_friction_sq_y_grid,
				...
				Statesptr, Parptr, Solverptr, SGCptr, Fptr, Locptr, Damptr, Super_linksptr, Arrptr,Infilptr,
				tmp_thread_data,
				verbose);
...
#pragma omp single
			{
				...
				if (curr_time + Solverptr->SGCtmpTstep > Solverptr->Sim_Time)
				{
					delta_time = Solverptr->Sim_Time - curr_time;
					//curr_time = Solverptr->Sim_Time;
					Solverptr->SGCtmpTstep = delta_time;
				}
				...
				if (curr_time >= Parptr->SaveTotal)
				{
					...
					write_regular_output(Fnameptr->resrootname, 
						...
						dx_col, dy_col,
						Solverptr, Statesptr, Parptr, boundary_cond, SGCptr,Infilptr,
						&Statesptr->output_params,
						Parptr->SaveNo,
						Statesptr->save_depth, Statesptr->save_elev,Statesptr->save_grnamp_infil_rate, Statesptr->save_Qs, 
	...
	const NUMERIC_TYPE delta_time, const NUMERIC_TYPE g, const NUMERIC_TYPE depth_thresh, const NUMERIC_TYPE max_Froude, WetDryRowBound * wet_dry_bounds)
{
	...
	for (i = 0; i < Super_linksptr->num_links; i++)
	{
		...
		NUMERIC_TYPE h1 = h_grid[Super_linksptr->link_index_2D[i]] + Super_linksptr->bfH[i]; // floodplain edge depth
		NUMERIC_TYPE z0 = Super_linksptr->SGC_z[i];
		NUMERIC_TYPE z1 = Super_linksptr->DEM_z[i];
		NUMERIC_TYPE w  = Super_linksptr->w[i];
		NUMERIC_TYPE dx = Super_linksptr->dx[i];
		...
		NUMERIC_TYPE q_tmp, surface_slope;
		if (hflow > depth_thresh && w > C(0.0))
		{
			NUMERIC_TYPE area = w* hflow;
			NUMERIC_TYPE dh = (surface_elevation0)-(surface_elevation1);
			surface_slope = -dh / dx;
