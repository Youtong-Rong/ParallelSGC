...
void Fast_MainInit(Fnames *Fnameptr, Files *Fptr, States *Statesptr, Pars *Parptr, Solver *Solverptr, BoundCs *BCptr, Stage *Locptr, ChannelSegmentType *ChannelSegments, Arrays *Arrptr, SGCprams *SGCptr, vector<ChannelSegmentType> *ChannelSegmentsVecPtr, DamData *Damptr, TGrnAmpt *Infilptr, int verbose)
{
	const int grid_cols = Parptr->xsz;
	const int grid_rows = Parptr->ysz;
	...
	int* rain_grid_ind = (int*)memory_allocate(sizeof(int) * grid_cols_padded * grid_rows); // rain grid index for UKCP rainfall mapping
	...
	NUMERIC_TYPE* soiltype_grid = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
	...
	if (Statesptr->calc_GrnAmpt_infiltration == ON) {
		Infilptr->F = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->Fu = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->Fumax = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->IMD = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->IMDmax = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->Infil_rate = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->Ks = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->Lu = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->S = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->T = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
		Infilptr->Sat = (NUMERIC_TYPE*)memory_allocate(sizeof(NUMERIC_TYPE) * grid_cols_padded * grid_rows);
	}
...
#pragma omp parallel for default(shared) schedule(static)
	...
	for (int block_index = 0; block_index < block_count; block_index++)
	{
		...
		for (int j = start_y; j < end_y; j++)
		{
			...
			if (Statesptr->calc_GrnAmpt_infiltration == ON)
			{
				memset(Infilptr->F + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->Fu + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->Fumax + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->IMD + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->IMDmax + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->Infil_rate + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->Ks + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->Lu + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->S + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->T + dest_row_index + grid_cols, 0, padding);
				memset(Infilptr->Sat + dest_row_index + grid_cols, 0, padding);

				memcpy(soiltype_grid + dest_row_index, Arrptr->SoilType + source_row_index, source_bytes_per_row);
				memset(soiltype_grid + dest_row_index + grid_cols, 0, padding);
				

				// the grnampt used evap_row, it can be always zero or can be defined within a file;in SGC2_Infil_Rate_floodplain_row() function evap_row 
				//should be considered if there is evaporation
	...
	Fast_IterateLoop(grid_cols, grid_rows, grid_cols_padded,
		...
		evap_time_series, &evap_grid, rain_time_series, rain_grid, rain_grid_ind, dist_infil_grid,
...
void Fast_MainStart(Fnames *Fnameptr, Files *Fptr, States *Statesptr, Pars *Parptr, Solver *Solverptr, BoundCs *BCptr, Stage *Locptr, ChannelSegmentType *ChannelSegments, Arrays *Arrptr, SGCprams *SGCptr, vector<ChannelSegmentType> *ChannelSegmentsVecPtr, DamData *Damptr, TGrnAmpt *Infilptr, int verbose)
{
	if (verbose == ON)
	{
		printf("\nStarting time steps: ");
...
#if (_NETCDF == 1)	
	if (verbose == ON) printf("NETCDF == on\n");
#endif
	...
	Fast_MainInit(Fnameptr, Fptr, Statesptr, Parptr, Solverptr, BCptr, Locptr, ChannelSegments, Arrptr, SGCptr, ChannelSegmentsVecPtr, Damptr, Infilptr, verbose);
}
void InitSuperLinksStructure(const int grid_rows, const int grid_cols, const int grid_cols_padded, SuperGridLinksList * Super_linksptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, SGCprams *SGCptr, Solver * Solverptr, Fnames * Fnameptr, int verbose)
{
	...
	else // calculate links in code using nearest neighbour
	{
		...
		for (j = 0; j < Parptr->ysz; j++) for (i = 0; i < Parptr->xsz; i++) // loop through model domain
		{
			...
			adjacent_to_chanmask = 0; // default assumption is not next to chanmask
			...
			else if (Arrptr->SGCwidth[p0] == C(0.0))// 2D domain cell ... identify all locations on a ChanMask edge that do not include a sub-grid channel when in coastal mode connects subgrid channels	 
			...
			else if (Statesptr->coastal_mode == ON)
			{
				// Data Needed: location(x,y), z, n, w, dx
				// Check north
				if (j > 0 && Arrptr->ChanMask[p0 - Parptr->xsz] == 1) adjacent_to_chanmask = 1;
				// Check south
				if (j + 1 < Parptr->ysz && Arrptr->ChanMask[p0 + Parptr->xsz] == 1) adjacent_to_chanmask = 1;
				// Check east
				if (i < Parptr->xsz - 1 && Arrptr->ChanMask[p0 + 1] == 1) adjacent_to_chanmask = 1;
				// Check west
				if (i > 0 && Arrptr->ChanMask[p0 - 1] == 1) adjacent_to_chanmask = 1;
				// Get the data needed
				if (adjacent_to_chanmask == 1)	n_links = n_links++;
			}
		}
		if (Statesptr->debugmode == ON) printf("Superlinks step 0.1\n");
		...
		int* tmp_SGC_mask_ind = new int[n_links]();
		int* tmp_2D_mask_ind = new int[n_links]();
		NUMERIC_TYPE* tmp_z_SGC = new NUMERIC_TYPE[n_SGC_under_mask]();
		NUMERIC_TYPE* tmp_z_2D = new NUMERIC_TYPE[n_links]();
		NUMERIC_TYPE* tmp_bfH_2D = new NUMERIC_TYPE[n_links]();
		...
		NUMERIC_TYPE* tmp_bfH_SGC = new NUMERIC_TYPE[n_SGC_under_mask]();

		// reset links counters to go again
		n_links = 0;
		n_SGC_under_mask = 0;
		if (Statesptr->debugmode == ON) printf("Superlinks step 1\n");
		...
		for (j = 0; j < Parptr->ysz; j++) for (i = 0; i < Parptr->xsz; i++) // loop through model domain
		{
			...
			{                             // Data Needed: location (x,y), bedz
				if (Arrptr->SGCwidth[p0] > C(0.0)) // identify if sub-grid channel cells // store location
				{
					...
					tmp_bfH_SGC[n_SGC_under_mask] = Arrptr->SGCbfH[p0];
					tmp_SGC_mask_ind[n_SGC_under_mask] = Arrptr->ChanMask[p0];
					n_SGC_under_mask = n_SGC_under_mask++;
				}
			}
			...
			{    // Data Needed: location(x,y), z, n, w, dx
				...
				{
					adjacent_to_chanmask = 1; dx_n = Arrptr->dy[p0]; w_n = Arrptr->dx[p0]; mask_ind = Arrptr->ChanMask[p0 - Parptr->xsz];
				}
				// Check south
				if (j + 1 < Parptr->ysz && Arrptr->ChanMask[p0 + Parptr->xsz] > 0)
				{
					adjacent_to_chanmask = 1; dx_s = Arrptr->dy[p0]; w_s = Arrptr->dx[p0]; mask_ind = Arrptr->ChanMask[p0 + Parptr->xsz];
				}
				// Check east
				if (i < Parptr->xsz - 1 && Arrptr->ChanMask[p0 + 1] > 0)
				{
					adjacent_to_chanmask = 1; dx_e = Arrptr->dx[p0]; w_e = Arrptr->dy[p0]; mask_ind = Arrptr->ChanMask[p0 + 1];
				}
				// Check west
				if (i > 0 && Arrptr->ChanMask[p0 - 1] > 0)
				{
					adjacent_to_chanmask = 1; dx_w = Arrptr->dx[p0]; w_w = Arrptr->dy[p0]; mask_ind = Arrptr->ChanMask[p0 - 1];
				}
				// Get the data needed
				if (adjacent_to_chanmask == 1)
				{
					...
					tmp_2D_mask_ind[n_links] = mask_ind;
					// get variables
			...
			else if (Statesptr->coastal_mode == ON && Arrptr->SGCwidth[p0] > C(0.0))
			{
				NUMERIC_TYPE dx_n = C(0.0), dx_e = C(0.0), dx_s = C(0.0), dx_w = C(0.0);
				NUMERIC_TYPE w_n = C(0.0), w_e = C(0.0), w_s = C(0.0), w_w = C(0.0);
				NUMERIC_TYPE max_dx, max_dy, max_wx, max_wy;
				int mask_ind = 0;
				// Check north
				if (j > 0 && Arrptr->ChanMask[p0 - Parptr->xsz] == 1)
				{
					adjacent_to_chanmask = 1; dx_n = Arrptr->dy[p0]; w_n = Arrptr->dx[p0]; mask_ind = Arrptr->ChanMask[p0 - Parptr->xsz];
				}
				// Check south
				if (j + 1 < Parptr->ysz && Arrptr->ChanMask[p0 + Parptr->xsz] == 1)
				{
					adjacent_to_chanmask = 1; dx_s = Arrptr->dy[p0]; w_s = Arrptr->dx[p0]; mask_ind = Arrptr->ChanMask[p0 + Parptr->xsz];
				}
				// Check east
				if (i < Parptr->xsz - 1 && Arrptr->ChanMask[p0 + 1] == 1)
				{
					adjacent_to_chanmask = 1; dx_e = Arrptr->dx[p0]; w_e = Arrptr->dy[p0]; mask_ind = Arrptr->ChanMask[p0 + 1];
				}
				// Check west
				if (i > 0 && Arrptr->ChanMask[p0 - 1] == 1)
				{
					adjacent_to_chanmask = 1; dx_w = Arrptr->dx[p0]; w_w = Arrptr->dy[p0]; mask_ind = Arrptr->ChanMask[p0 - 1];
				}
				// Get the data needed
				if (adjacent_to_chanmask == 1)
				{
					...
					// get variables
					tmp_z_2D[n_links] = Arrptr->SGCz[p0]; // use SGC bed
					tmp_gn2[n_links] = (Parptr->SGC_n* Parptr->SGC_n) * Solverptr->g ; // use SGC manning's n
					tmp_w[n_links] = Arrptr->SGCwidth[p0]; // use SGC width
					tmp_bfH_2D[n_links] = Arrptr->SGCbfH[p0]; // use SGC bfH
					tmp_dx[n_links] = FMAX(FMAX(dx_n, dx_s), FMAX(dx_e, dx_w)); // use longest dx
					n_links = n_links++; // add one to number of links
				}
			}
		}

		// Step 2: Allocate memory
		if (Statesptr->debugmode == ON) printf("Superlinks step 2\n");
		AllocateSuperLinksMemory(n_links, Super_linksptr);

		// Step 3: for each edge identify closest sug_grid cell for linking list
		if (Statesptr->debugmode == ON) printf("Superlinks step 3\n");
#pragma omp for private (j)
		for (i = 0; i < n_links; i++)
		{
			...
			Super_linksptr->bfH[i] = tmp_bfH_2D[i];
			...
			for (j = 0; j < n_SGC_under_mask; j++) // loop through sub-grid cells
			{
				...
				if (tmp_2D_mask_ind[i] == tmp_SGC_mask_ind[j]) // only link to edges to channels with the same mask id
				{ 
			...
			if (dist == 2000000000) // no link found
			{
				Super_linksptr->w[i] = C(0.0); // set width to zero to prevent any flow
				if (verbose == ON) printf("Warning: no link found for supergrid edge %i. set to zero flow\n", i);
			}
			...
			Super_linksptr->SGC_bfH[i] = tmp_bfH_SGC[closest_line];
		}
		if (verbose == ON)
		{
		...
		{ 
			//Write the superlinks list to file
			WriteSuperlinksToFile(n_links, Fnameptr->resrootname, Super_linksptr);
...
void AllocateSuperLinksMemory(int n_links, SuperGridLinksList * Super_linksptr)
{
	...
	Super_linksptr->bfH = (NUMERIC_TYPE*)memory_allocate(n_links * sizeof(NUMERIC_TYPE));
...
void WriteSuperlinksToFile(const int n_links, const char* resrootname, const SuperGridLinksList* Super_linksptr)
{
	int i;
	FILE* fp;
	char filename[512];
	
	printf("Write superlinks debug file...  ");
	sprintf(filename, "%s_SLL.txt", resrootname);
	...
	for (i = 0; i < n_links; i++)
	{
		fprintf(fp, "%i %i %i %i %.3" NUM_FMT" %.3" NUM_FMT" %.3" NUM_FMT" %.3" NUM_FMT" %.3" NUM_FMT" %.3" NUM_FMT" \n", Super_linksptr->link_index_2D_i[i], Super_linksptr->link_index_2D_j[i], Super_linksptr->link_index_SGC_i[i], Super_linksptr->link_index_SGC_j[i], Super_linksptr->DEM_z[i], Super_linksptr->SGC_z[i], Super_linksptr->SGC_bfH[i], Super_linksptr->w[i], Super_linksptr->dx[i], Super_linksptr->gn2[i]);
	}
