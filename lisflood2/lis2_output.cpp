...
void write_regular_output(const char * resrootname, const int grid_cols, const int grid_rows, const int grid_cols_padded,
	...
	Solver *Solverptr, States *Statesptr, Pars *Parptr, BoundaryCondition *boundary_cond, SGCprams *SGCptr,TGrnAmpt* Infilptr,
	OutputParams * output_params,
	const int save_number,
	const int save_depth,
	const int save_elev,
	const int save_grnamp_infil_rate,
	const int save_Qs,
	...
	const int save_sub_grid_Velocity)
{
	...
	else
	{
		...
		if (save_grnamp_infil_rate == ON)
		{
			bool is_infil_rate_calculated = false;
			for (int j = 0; j < grid_rows; j++)
			{
				for (int i = 0; i < grid_cols; i++)
				{
					...
					NUMERIC_TYPE temp=Infilptr->Infil_rate[index_padded];
				
					
					if (temp > C(0.0))
					{
						temp = Infilptr->Infil_rate[index_padded] * C(3600.) * C(1000.);
						is_infil_rate_calculated = true;
					}
					
					tmp_grid1[index] = temp;
				}
			}
			//if(is_infil_rate_calculated)
			//{
			write_grid(resrootname, save_number,
				output_params->netcdf_state.varid_depth, ".infil",
				tmp_grid1, grid_cols, grid_rows, Parptr->blx, Parptr->bly, Parptr->dx, output_params);
			//}
		}
