...
void LoadSoilType(Fnames* Fnameptr, States* Statesptr, Pars* Parptr, Arrays* Arrptr, Solver* Solverptr, const int verbose)
{
	...
	//FILE* fp = LoadDomainGeometry(Fnameptr->soiltypefilename, Parptr, verbose, no_data_value);
	char dum[800];
	Solverptr->new_infil_time = memory_allocate_zero_numeric_legacy(Parptr->ysz);
	if (Solverptr->t > C(0.0))
	{
		for (int j = 0; j < Parptr->ysz; j++)
		{
			Solverptr->new_infil_time[j] = Solverptr->t;
		}
		
	}
	Arrptr->SoilType = memory_allocate_numeric_legacy(Parptr->xsz * Parptr->ysz);
	FILE* fp = fopen_or_die(Fnameptr->soiltypefilename, "rb", "Loading SoilType\n", verbose);
	...
	LoadDEMData(Parptr, Arrptr->SoilType, fp, no_data_value);
	fclose(fp);
