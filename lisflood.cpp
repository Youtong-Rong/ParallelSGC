...
int main(int argc, char *argv[])
{
	...
	memset(&Infil, 0, sizeof(TGrnAmpt));
	...
	Params.UKCP = OFF; // flag to say dynamic rainfall uses UKCP data format in NetCDF file (JCN)
	...
	ParSolver.itCount = 0;
	ParSolver.t = C(0.0);
	ParSolver.wet_infil_time = C(300.0);    
	ParSolver.dry_infil_time = C(1200.0);
	ParSolver.min_infil_time_step = ParSolver.wet_infil_time;
	...
	SimStates.calc_GrnAmpt_infiltration = OFF;
	SimStates.save_grnamp_infil_rate = OFF;  //output the green-ampt infiltration rate with the same frequency as the depth output
   ...
   // }
	...
	if (Statesptr->calc_GrnAmpt_infiltration == ON) LoadSoilType(Fnameptr, Statesptr, Parptr, Arrptr, Solverptr, verbosemode);
	...
	{
		Fast_MainStart(Fnameptr, &Fps, Statesptr, Parptr, Solverptr, BCptr, Stageptr, CSTypePtr, Arrptr, SGCptr, ChannelSegmentsVecPtr, Damptr,Infilptr, verbosemode); //Damptr added by FEOL
