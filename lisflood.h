...
struct NetCDFVariable
{
  ...
  NUMERIC_TYPE* ys;
  NUMERIC_TYPE* data;
  NUMERIC_TYPE* latitude_UKCP;
  NUMERIC_TYPE* longitude_UKCP;
  int UKCP;
};
...
struct States{
	...
	int dg2;
	int dynamicrainfall;
	int coastal_mode; // turns on supergrid model coastal mode
};
...
struct Solver{
	NUMERIC_TYPE t;
	NUMERIC_TYPE* new_infil_time;
	NUMERIC_TYPE wet_infil_time;
	NUMERIC_TYPE dry_infil_time;
	NUMERIC_TYPE min_infil_time_step;
	NUMERIC_TYPE g;
	NUMERIC_TYPE divg;
	NUMERIC_TYPE cfl;
...
struct TGrnAmpt {
	NUMERIC_TYPE* S;   // Capillary suction head (m)
	NUMERIC_TYPE* Ks;    // Sat. hyd. conductivity (m/sec)
	NUMERIC_TYPE* IMDmax; // Max. init. moisture deficit
	NUMERIC_TYPE* Lu;     //depth of upper soil zone (m)
...
void LoadSoilType(Fnames* Fnameptr, States* Statesptr, Pars* Parptr, Arrays* Arrptr, Solver* Solverptr, const int verbose);
void LoadSoilIDData(Pars* Parptr, NUMERIC_TYPE* SoilType, FILE* fp, NUMERIC_TYPE file_nodata_value);
