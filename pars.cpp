...
void CheckParam(char* param_name, char* param_value_ptr, int line_number, Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Solver *Solverptr, int verbose, int mode)
{
	...
	if (read_string_param(param_name, line_number, param_value_ptr, "DEMfile", Fnameptr->demfilename, verbose, mode))
		return;
	if (read_string_param(param_name, line_number, param_value_ptr, "soiltypefile", Fnameptr->soiltypefilename, verbose, mode))
		return;
	...
	if (read_numeric_param(param_name, line_number, param_value_ptr, "sim_time", &Solverptr->Sim_Time, verbose, mode) ||
		...
		return;
	if (read_numeric_param(param_name, line_number, param_value_ptr, "wet_infil_time", &Solverptr->wet_infil_time, verbose, mode) ||
		read_numeric_param(param_name, line_number, param_value_ptr, "wetinfiltime", &Solverptr->wet_infil_time, verbose, mode))
		return;

	if (read_numeric_param(param_name, line_number, param_value_ptr, "dry_infil_time", &Solverptr->dry_infil_time, verbose, mode) ||
		read_numeric_param(param_name, line_number, param_value_ptr, "dryinfiltime", &Solverptr->dry_infil_time, verbose, mode))
		return;
	...
	if (read_empty_param(param_name, line_number, param_value_ptr, "grnampt_infil_rate", verbose, mode))
	{
		Statesptr->save_grnamp_infil_rate = ON;
		printf("\nAttention: GrnAmpt infiltration rate in *.infil file has the unit of mm/hr\n");
		return;
	}
	...
	if (read_empty_param(param_name, line_number, param_value_ptr, "coastal_mode", verbose, mode))
	{
		Statesptr->coastal_mode = ON;
		return;
	}
	...
	if (read_empty_param(param_name, line_number, param_value_ptr, "grnampt_infil", verbose, mode))
	{
		Statesptr->calc_GrnAmpt_infiltration = ON;
		return;
	}
	...
	if (read_empty_param(param_name, line_number, param_value_ptr, "UKCP", verbose, mode))
	{
		Parptr->UKCP = ON;
#if (_NETCDF != 1)			
		fprintf(stderr, "ERROR: dynamic rain UKCP requires lisflood to be compiled with NetCDF support\n");
		exit(1);
#endif
		return;
	}
...
void CheckParams(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Solver *Solverptr, int verbose)
{
	...
	if (Statesptr->save_grnamp_infil_rate == ON && Statesptr->calc_GrnAmpt_infiltration == OFF) //FEOL
	{
		printf("\nWARNING: GrnAmpt infiltration model is not activated while demanding save grnamp infiltration rate. Aborting ....\n\n");
		exit(1);
	}
